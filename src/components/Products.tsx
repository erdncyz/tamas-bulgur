          ...
          {productsData.map((product, index) => (
            ...
            >
              <div className="relative aspect-square overflow-hidden rounded-2xl mb-6 bg-brand-cream dark:bg-gray-800">
                ...
                />
                <div className="absolute inset-0 bg-brand-brown/20 dark:bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <motion.div
                    whileHover={{ scale: 1.1 }}
                    className="bg-white dark:bg-brand-green text-brand-brown dark:text-white p-4 rounded-full shadow-lg"
                  >
                    <ArrowRight size={20} />
                  </motion.div>
                </div>
              </div>
              <h3 className="text-2xl font-serif font-bold text-brand-brown dark:text-white mb-3 tracking-wide">
                {product.name}
              </h3>
              <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed mb-4">
                {product.description}
              </p>
              <div className="h-[1px] w-12 bg-brand-green group-hover:w-full dark:group-hover:w-full transition-all duration-300" />
            </motion.div>
          ))}
        </div>
      </div>
